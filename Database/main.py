...
def insertDB(replies_df,retweets_df, tweets_df):
    ...
    try: 
        ...
        print("Could not connect to MongoDB-Replies could not be inserted")
    ...
    try: 
        ...
        cluster = MongoClient("mongodb+srv://colombia:<password>@colombia1.vp4wl.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
        print("Connected") 
        db = cluster["RetweetsDB"] #put your db name
        collection = db["RetweetsC"] #put your collection name
        records = json.loads(retweets_df.T.to_json()).values()
        collection.insert(records)
        print("Retweets Inserted.")
        
    except:   
        print("Could not connect to MongoDB-Retweets could not be inserted")
    ...
    try: 
        ...
        cluster = MongoClient("mongodb+srv://colombia:<password>@colombia1.vp4wl.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
        print("Connected") 
        db = cluster["TweetsDB"] #put your db name
        collection = db["TweetsC"] #put your collection name
        records = json.loads(tweets_df.T.to_json()).values()
        collection.insert(records)
        print("Tweets Inserted.")
        
    except:   
        print("Could not connect to MongoDB-Tweets could not be inserted")
...
if __name__ == "__main__":
    ...
    for i in list(replies_l[0].keys()):
      print("key: {}, type: {}".format(i,type(replies_l[0][i])))
    ...
    #Undo-Comment if you want to push to db
