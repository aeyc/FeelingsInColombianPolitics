...
if __name__ == "__main__":
    ...
    for profile in profiles:
        ...
        for tweet in tw.Paginator(client.get_users_tweets,id = user_id, tweet_fields=["author_id","created_at","public_metrics","entities","geo"], exclude=["retweets", "replies"],start_time=fromDate.strftime("%Y-%m-%dT%H:%M:%SZ")).flatten(limit=RESULTS_LIMIT):
    ...
    for tweet_id in tweets_id:
        ...
        for reply in tw.Paginator(client.search_recent_tweets,query="conversation_id:{}".format(tweet_id),tweet_fields=["author_id","created_at","public_metrics","entities","geo"], max_results=10).flatten(limit=RESULTS_LIMIT):
            parsed = {}
            parsed['id'] = reply.id
